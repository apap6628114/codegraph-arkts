...
- **SQLite 后端** — 新增 `better-sqlite3` 后备：当内置 `node:sqlite` 不可用或缺 FTS5 时自动回退（部分 Windows Node 发行版适用）
...
- Node.js 22.5+（用于内置 `node:sqlite` + FTS5），或安装 `better-sqlite3` 作为后备
...
# 4. 安装 MCP 服务器（以 Claude Code 为例，项目级配置）
...
源码 (.ets, .ts, .py ...)
  → tree-sitter AST
    → LanguageExtractor（语言提取器配置）
      ...
      → Edges（调用、包含、装饰、继承）
        → SQLite 知识图谱
          → MCP 工具（search、explore、node、callers、callees）
            → AI 编程助手
```
...
语言特定的钩子（`resolveBody`、`getVisibility` 等）处理 ArkTS 的 AST 特殊之处，如 `export function` 中 `function_signature` 与函数体的分离、箭头函数类字段的 body 解析、`accessibility_modifier` 可见性识别。
...
├── db/
│   └── sqlite-adapter.ts     ← SQLite 后端（新增 better-sqlite3 后备）
└── types.ts                  ← 语言类型定义
```

## 上游

此分支是 [colbymchenry/codegraph](https://github.com/colbymchenry/codegraph) 的分支，新增了 ArkTS 支持。ArkTS 的 tree-sitter 语法来自 [harmony-contrib/tree-sitter-arkts](https://github.com/harmony-contrib/tree-sitter-arkts)。
...
完整的 CLI 参考、框架路由识别、基准测试结果和故障排查请参阅[上游 README](https://github.com/colbymchenry/codegraph)（英文）。

## 许可证

MIT — 参见 [LICENSE](LICENSE)。上游 CodeGraph 由 Colby Mchenry 开发。ArkTS 语法源自 tree-sitter-typescript 与 harmony-contrib 的 tree-sitter-arkts。
